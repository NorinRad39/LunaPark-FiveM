...
using System.Drawing; // Pour SizeF, Color, PointF, etc.
using Font = CitizenFX.Core.UI.Font; // Résout l'ambiguïté sur Font
    ...
    public class TabInteractiveListItem : TabItem
    {
        ...
        public override void Draw()
        {
            ...
            for (int c = _minItem; c < Math.Min(Items.Count, _maxItem); c++)
            {
                ...
                ((CitizenFX.Core.UI.Rectangle)new UIResRectangle(base.SafeSize.AddPoints(new PointF(0f, (itemSize.Height + 3f) * (float)i)), itemSize, (Index == c && Focused) ? Color.FromArgb(fullAlpha, Colors.White) : ((Focused && hovering) ? Color.FromArgb(100, 50, 50, 50) : Color.FromArgb(blackAlpha, Colors.Black)))).Draw();
                ...
                else
                {
                    ...
                    if (convItem != null)
                    {
                        ...
                        itemText.Color = (!convItem.Enabled) ? Color.FromArgb(163, 159, 148) : (selected ? Colors.Black : Colors.WhiteSmoke);
                        ...
                        if (selected)
                        {
                            arrowLeft.Draw();
                            arrowRight.Draw();
                            itemText.Position = base.SafeSize.AddPoints(new PointF(base.BottomRight.X - base.SafeSize.X - 30f, (float)yoffset + (itemSize.Height + 3f) * (float)i));
                        }
                        else
                        {
                            itemText.Position = base.SafeSize.AddPoints(new PointF(base.BottomRight.X - base.SafeSize.X - 5f, (float)yoffset + (itemSize.Height + 3f) * (float)i));
                        }
                        ((Text)itemText).Draw();
                    }
                }
