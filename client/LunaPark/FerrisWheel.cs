...
namespace Client.net.LunaPark
{
    internal class FerrisWheel : BaseScript
    {
        ...
        public FerrisWheel()
        {
            ...
            EventHandlers.Add("onResourceStop", (Delegate)new Action<string>(OnStop));
            EventHandlers.Add("FerrisWheel:forceState", (Delegate)new Action<string>(ForceState));
            EventHandlers.Add("FerrisWheel:UpdateCabins", (Delegate)new Action<int, int>(UpdateCabins));
            EventHandlers.Add("FerrisWheel:StopWheel", (Delegate)new Action<bool>(WheelState));
            EventHandlers.Add("FerrisWheel:playerGetOn", (Delegate)new Action<int, int>(PlayerGetOn));
            EventHandlers.Add("FerrisWheel:playerGetOff", (Delegate)new Action<int, int>(PlayerGetOff));
            EventHandlers.Add("FerrisWheel:UpdateGradient", (Delegate)new Action<int>(UpdateGradient));
        ...
        private async Task SpawnaWheel()
        {
            ...
            ((Entity)val).Position = new Vector3(-1663.97f, -1126.7f, 30.7f);
            ((Entity)val).Rotation = new Vector3(360f, 0f, 0f);
            ((Entity)val).IsPositionFrozen = true;
            ((Entity)val).LodDistance = 1000;
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                ((Entity)val2).IsInvincible = true;
                ((Entity)val2).Position = func_147(i);
                ((Entity)val2).LodDistance = 1000;
                ((Entity)val2).IsPositionFrozen = true;
                obj.Entity = val2;
                Cabins[i].Index = i;
            }
            await Task.Yield(); // Ajout d'un await pour éviter CS1998
