...
namespace Client.net.LunaPark
{
	public class UITimerBarItem
	{
		...
		{
			Text = new UIResText((text != string.Empty) ? text : "N/A", new PointF(0f, 0f), 0.35f, Color.FromArgb(255, 255, 255, 255), CitizenFX.Core.UI.Font.ChaletLondon, (Alignment)2);
		}
		...
		public UITimerBarItem(string text, string txtDictionary, string txtName, Color color)
		{
			Background = new Sprite("timerbars", "all_black_bg", new PointF(0f, 0f), new SizeF(350f, 35f), 0f, color);
			Text = new UIResText((text != string.Empty) ? text : "N/A", new PointF(0f, 0f), 0.35f, Color.FromArgb(255, 255, 255, 255), CitizenFX.Core.UI.Font.ChaletLondon, (Alignment)2);
			TextTimerBar = new UIResText("", new PointF(0f, 0f), 0.35f, Color.FromArgb(255, 255, 255, 255), CitizenFX.Core.UI.Font.ChaletLondon, (Alignment)2);
		...
		public virtual async void Draw(float offset)
		{
			if (Enabled)
			{
				...
				Background.Draw();
				Text.Position = new PointF(Position.X + 170f, Position.Y - offset);
				Text.Draw();
				if (TextTimerBar.Caption != "")
				{
					TextTimerBar.Position = new PointF(Position.X + 340f, Position.Y - offset);
					TextTimerBar.Draw();
				}
				for (int i = 6; i < 10; i++)
				{
