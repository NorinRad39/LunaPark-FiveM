...
namespace Client.net.LunaPark
{
    public class RollerCoaster : BaseScript
    {
        ...
        {
            EventHandlers.Add("RollerCoaster:forceState", new Action<string>(ForceState));
            EventHandlers.Add("RollerCoaster:playerGetOn", new Action<int, int, int>(playerGetOn));
            EventHandlers.Add("RollerCoaster:playerGetOff", new Action<int>(playerGetOff));
            EventHandlers.Add("RollerCoaster:SyncCars", new Action<int, int>(SyncCars));
            EventHandlers.Add("onResourceStop", new Action<string>(OnStop));

            Blip val = new Blip(API.AddBlipForCoord(-1651.641f, -1134.325f, 21.90398f));
            val.Sprite = (BlipSprite)266;
            val.IsShortRange = true;
            val.Name = "RollerCoaster";
            Blip roller = val;

            API.SetBlipDisplay(roller.Handle, 4);

            CaricaTutto();

            // Abonnements Tick
            Tick += DeleteYellows;
            Tick += TimerBarHandler;

            TimerBarPool.Add(RollerBar);
        }

        private Task SpawnaMontagne()
        {
            func_191();
        ...
        private async void CaricaTutto()
        {
            ...
            // Abonnements Tick
            Tick += MoveRollerCoaster;
            Tick += ControlloMontagne;
        }

        private Task TimerBarHandler()
        {
            if (TimerBarPool.TimerBars.Count > 0)
            {
                return TimerBarPool.Draw();
            }
            return Task.CompletedTask;
        }
        ...
        private async Task DeleteYellows()
        {
            try
            {
                MRClosest = World.GetAllProps()
                    ...
                    .Where(o => DaEliminare.Contains((ObjectHash)o.Model.Hash))
                    .FirstOrDefault(o =>
                    {
                        Vector3 position = o.Position;
                        return position.DistanceToSquared(Game.PlayerPed.Position) < Math.Pow(600.0, 2.0);
                    });

                if (MRClosest != null && MRClosest.Exists())
                {
                    MRClosest.Delete();
                }
            }
        ...
        private async Task MoveRollerCoaster()
        {
            try
            {
                ...
                {
                    case "WAITING":
                        ((Text)RollerBar.TextTimerBar).Caption = $"{tempo} sec.";
                        while (tempo > 0)
                        {
                    ...
                    case "DEPARTING":
                        ...
                        {
                            API.PlayEntityAnim(o.Entity.Handle, "safety_bar_enter_roller_car", RollerAnim, 8f, false, true, false, 0f, 0);
                        });
                        API.PlaySoundFromEntity(-1, "Bar_Lower_And_Lock", Roller.Cars[1].Entity.Handle, "DLC_IND_ROLLERCOASTER_SOUNDS", false, 0);
                        if (!ImSitting)
                        {
                    ...
                    case "TRIP":
                        if (ValueUnknownIndex != 0)
                        {
                            ...
                            if (ImSitting)
                            {
                                ...
                                if (!API.IsEntityPlayingAnim(API.PlayerPedId(), RollerAnim, "safety_bar_grip_move_a_player_" + Place, 3) &&
                                    !API.IsEntityPlayingAnim(API.PlayerPedId(), RollerAnim, "hands_up_idle_a_player_" + Place, 3) &&
                                    !API.IsEntityPlayingAnim(API.PlayerPedId(), RollerAnim, "hands_up_idle_a_player_" + Place, 3) &&
                                    !API.IsEntityPlayingAnim(API.PlayerPedId(), RollerAnim, "hands_up_exit_player_" + Place, 3))
                                {
                                    Game.PlayerPed.Task.PlayAnimation(RollerAnim, "safety_bar_grip_move_a_player_" + Place, 8f, -1, (AnimationFlags)1);
                                }
                                ...
                                {
                                    Game.PlayerPed.Task.PlayAnimation(RollerAnim, "hands_up_enter_player_" + Place, 8f, -1, (AnimationFlags)2);
                                    while (API.IsEntityPlayingAnim(API.PlayerPedId(), RollerAnim, "hands_up_enter_player_" + Place, 3))
                                    {
                                        await BaseScript.Delay(0);
                                    }
                                    Game.PlayerPed.Task.PlayAnimation(RollerAnim, "hands_up_idle_a_player_" + Place, 8f, -1, (AnimationFlags)1);
                                }
                                else if (API.IsEntityPlayingAnim(API.PlayerPedId(), RollerAnim, "hands_up_idle_a_player_" + Place, 3))
                                {
                                    Game.PlayerPed.Task.PlayAnimation(RollerAnim, "hands_up_exit_player_" + Place, 8f, -1, (AnimationFlags)2);
                            ...
                            {
                                API.PlayStreamFromObject(Roller.Cars[2].Entity.Handle);
                            }
                        }
                        else
                        {
                        ...
                        break;
                    case "ARRIVAL":
                        API.PlaySoundFromEntity(-1, "Ride_Stop", Roller.Cars[1].Entity.Handle, "DLC_IND_ROLLERCOASTER_SOUNDS", false, 0);
                        if (API.IsEntityPlayingAnim(API.PlayerPedId(), RollerAnim, "hands_up_idle_a_player_" + Place, 3))
                        {
                            Game.PlayerPed.Task.PlayAnimation(RollerAnim, "hands_up_exit_player_" + Place, 8f, -1, (AnimationFlags)2);
                    ...
                    case "STOP":
                        ...
                        else if (Active)
                        {
                            ...
                            Roller.Cars.ToList().ForEach(delegate (RollerCar o)
                            {
                                API.PlayEntityAnim(o.Entity.Handle, "safety_bar_exit_roller_car", RollerAnim, 8f, false, true, false, 0f, 0);
                            });
                            API.PlaySoundFromEntity(-1, "Bar_Unlock_And_Raise", Roller.Cars[1].Entity.Handle, "DLC_IND_ROLLERCOASTER_SOUNDS", false, 0);
                            if (ImSitting)
                            {
                                BaseScript.TriggerServerEvent("RollerCoaster:playerGetOff", new object[1] { Game.PlayerPed.NetworkId });
                            }
                            Roller.Cars.ToList().ForEach(delegate (RollerCar o)
                            {
                                o.Occupied = 0;
        ...
        private void SyncCars(int carrello, int Occupied)
        {
        ...
        private async void playerGetOn(int playernetid, int index, int carrelloHash)
        {
            ...
            if (personaggio.NetworkId != Game.PlayerPed.NetworkId && !API.NetworkHasControlOfNetworkId(playernetid))
            {
            ...
            API.TaskGoStraightToCoord(personaggio.Handle, Coord.X, Coord.Y, Coord.Z, 1f, -1, 229.3511f, 0.2f);
            await BaseScript.Delay(1000);
            ...
            API.NetworkAddPedToSynchronisedScene(personaggio.Handle, iLocal_1443, RollerAnim, "enter_player_" + Place, 8f, -8f, 131072, 0, 1.14884608E+09f, 0);
            ...
            if (API.GetSynchronizedScenePhase(iVar1) > 0.99f)
            {
                ...
                API.NetworkAddPedToSynchronisedScene(personaggio.Handle, iLocal_1443, RollerAnim, "idle_a_player_" + Place, 8f, -8f, 131072, 0, 1.14884608E+09f, 0);
                API.NetworkStartSynchronisedScene(iLocal_1443);
            }
            await BaseScript.Delay(5000);
            Vector3 vVar0 = API.GetOffsetFromEntityGivenWorldCoords(Carrello.Handle, personaggio.Position.X, personaggio.Position.Y, personaggio.Position.Z);
            API.AttachEntityToEntity(personaggio.Handle, Carrello.Handle, 0, vVar0.X, vVar0.Y, vVar0.Z, 0f, 0f, personaggio.Heading - 139.96f, false, false, false, false, 2, true);
        ...
        private async void playerGetOff(int playernetid)
        {
            ...
            int iLocal_1443 = API.NetworkCreateSynchronisedScene(Coord.X, Coord.Y, Coord.Z, 0f, 0f, 139.96f, 2, true, false, 1.06535322E+09f, 0f, 1.06535322E+09f);
            API.NetworkAddPedToSynchronisedScene(personaggio.Handle, iLocal_1443, RollerAnim, "safety_bar_exit_player_" + Place, 8f, -8f, 131072, 0, 1.14884608E+09f, 0);
            API.NetworkStartSynchronisedScene(iLocal_1443);
            ...
            API.NetworkAddPedToSynchronisedScene(personaggio.Handle, iLocal_1442, RollerAnim, "exit_player_" + Place, 8f, -8f, 131072, 0, 1.14884608E+09f, 0);
            API.NetworkStartSynchronisedScene(iLocal_1442);
            await BaseScript.Delay(7000);
            personaggio.Task.ClearAll();
        }

        private Task ControlloMontagne()
        {
            foreach (Vector3 v in GetIn)
            {
                if (Game.PlayerPed.IsInRangeOf(v, 1.3f) && Roller.State == "WAITING" && tempo > 0)
                {
                    ...
                    if (Game.IsControlJustPressed(0, (Control)51))
                    {
                        ...
                        Coord = API.GetOffsetFromEntityInWorldCoords(Roller.Cars[GetIn.IndexOf(v) / 2].Entity.Handle, 0f, fVar2, 0f);
                        BaseScript.TriggerServerEvent("RollerCoaster:playerGetOn", new object[3]
                        {
                            Game.PlayerPed.NetworkId,
                            GetIn.IndexOf(v) / 2,
                            Roller.Cars[GetIn.IndexOf(v) / 2].Entity.Model.Hash
                        });
                    }
                }
            }
            ...
            {
                RollerBar.Enabled = Roller.State == "WAITING" && tempo > 0;
            }
            else
            {
        ...
        private async void func_191()
        {
            for (int iVar0 = 0; iVar0 < Roller.Cars.Length; iVar0++)
            {
                ...
                API.FreezeEntityPosition(Roller.Cars[iVar0].Entity.Handle, true);
                API.SetEntityLodDist(Roller.Cars[iVar0].Entity.Handle, 300);
                API.SetEntityInvincible(Roller.Cars[iVar0].Entity.Handle, true);
            }
            Roller.Speed = Roller.Value3[1];
            func_46(bParam0: false);
            Roller.Cars.ToList().ForEach(delegate (RollerCar o)
            {
                API.PlayEntityAnim(o.Entity.Handle, "idle_a_roller_car", RollerAnim, 8f, true, false, false, 0f, 0);
            });
            iLocal_715 = 0;
            func_133();
        }
        ...
        private async void func_133()
        {
            for (int iVar0 = 0; iVar0 < Roller.Cars.Length; iVar0++)
            {
                int iVar1 = await func_138(Roller.Speed - 2.55f * (float)iVar0, ValueUnknownIndex);
                Vector3 vVar2 = func_50(Roller.Speed - 2.55f * (float)iVar0, iVar1);
                API.SetEntityCoordsNoOffset(Roller.Cars[iVar0].Entity.Handle, vVar2.X, vVar2.Y, vVar2.Z, true, false, false);
                func_134(iVar0, iVar1, Roller.Speed - 2.55f * (float)iVar0);
            }
        }
        ...
        private void func_134(int iParam0, int iParam1, float fParam2)
        {
            ...
            API.SetEntityQuaternion(Roller.Cars[iParam0].Entity.Handle, uVar7[0], uVar7[1], uVar7[2], uVar7[3]);
            if (Game.PlayerPed.IsInRangeOf(Roller.Cars[0].Entity.Position, 50f) && iParam0 == 0 && iVar0 % 3 == 0)
            {
        ...
        private void func_220()
        {
            ...
            func_226(new Vector3(-1698.207f, -1131.882f, 27.6230f));
            ...
            func_226(new Vector3(-1669.649f, -1138.865f, 23.4720f));
            ...
            func_226(new Vector3(-1675.523f, -1122.444f, 21.3270f));
            ...
            func_226(new Vector3(-1693.069f, -1123.218f, 21.6720f));
            ...
            func_226(new Vector3(-1693239f, -1151.881f, 22.1555f));
            ...
            func_226(new Vector3(-1675.088f, -1158.437f, 20.2097f));
